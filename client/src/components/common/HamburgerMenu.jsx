...
const HamburgerMenu = ({ isMenuOpen, toggleMenu }) => {
  ...
  return (
    <div className="relative">
      <button
        id="hamburger-button"
        className={`text-2xl cursor-pointer relative w-6 h-6 z-50 ${isMenuOpen ? 'toggle-btn' : ''}`}
        onClick={toggleMenu}
      >
      ...
      {isMenuOpen && (
        <div className="menu-content animate-bounceIn fixed top-0 left-0 w-full h-full bg-white z-40">
          <ul className="flex flex-col items-center justify-center h-full">
            ...
            <hr className="w-3/4 my-4 border-t-2 border-gray-300" />

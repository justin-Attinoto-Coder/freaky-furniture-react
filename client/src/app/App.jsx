...
function App() {
    ...
    const fetchRecommendedItems = () => {
        axios.get(`${API_URL}/api/recommended`)
            .then(response => {
    ...
    const updateCartItem = (urlSlug, quantity) => {
        axios.put(`${API_URL}/api/cart/${urlSlug}`, { quantity })
            .then(() => {
                fetchCartItems();
            })
            .catch(error => {
    ...
    const deleteCartItem = (urlSlug) => {
        axios.delete(`${API_URL}/api/cart/${urlSlug}`)
            .then(() => {
                fetchCartItems();
            })
            .catch(error => {
    ...
    const clearCartAfterCheckout = () => {
        console.log('Cart cleared in the frontend');
        setCartItems([]);
    };
